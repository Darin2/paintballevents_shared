...
function printAllFieldsAsCards() {
    ...
    $sql = "SELECT fieldID, paintballFieldName, paintballFieldWebsite, paintballFieldWebsiteEventPage, 
        ...
        isPublished, longitude, latitude
        FROM `fields` 
    ...
    if ($stmt->prepare($sql)) {
        ...
        $stmt->bind_result($fieldID, $name, $website, $eventPage, $fullAddress, 
                   $googleMap, $facebook, $instagram, $tiktok, $isPublished, $longitude, $latitude);
...
function countFields() {
    ...
    $sql = "SELECT COUNT(*) FROM `fields` WHERE country_code = 'FRA'";
    $stmt = $conn->stmt_init();
...
<body>
    ...
    <h1 class="text-white mx-auto mt-5">🇫🇷 French fields (<?php echo countFields(); ?> total)</h1>
